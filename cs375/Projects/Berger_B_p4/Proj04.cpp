...
int main(int argc, char *argv[]){
	...
	if(argument== "0"){
		...
		int x, y;
		for(int i = 1; i <= numCards; i++){
			for(int k =1; k <= cash; k++){
				if(first){
					if(her[herNames[i]] <= k){
						x = row1[k - her[herNames[i]]] + market[herNames[i]];
						y = row1[k];
						if(x > y){
							row2[k] = x;
						} else{
					...
					if(her[herNames[i]] <= k){
						x = row2[k - her[herNames[i]]] + market[herNames[i]];
						y = row2[k];
						if(x > y){
							row1[k] = x;
						} else{
			...
			// for(int p = 0; p < row1.size(); p++){
			// 	row1[p] = row2[p];
			// }
		}
		time(&then);
		outFile << "Dynamic Programming: ";
		outFile << numCards << " ";
		outFile << row1[cash] << " ";
		outFile << difftime(now,then) << endl;


		//building entire table

		//vector<int> cardsIn;
		// vector<vector<int> > dynam;
		// dynam.resize(numCards + 1);
		// for(int i = 0; i <= numCards; i++){
		// 	dynam[i].resize(cash + 1);
		// }
		// int x, y;
		// for(int i = 1; i <= numCards; i++){
		// 	for(int k = 1; k <= cash; k++){
		// 		if(her[herNames[i]] <= k){
		// 			x = dynam[i - 1][k - her[herNames[i]]] + market[herNames[i]];
		// 			y = dynam[i-1][k];
		// 			if(x > y){
		// 				dynam[i][k] = x;
		// 			} else {
		...
		// 			}
		// 		} else {
		// 			dynam[i][k] = dynam[i - 1][k];
		// 		} 
		// 	}
		// }
		// cout << dynam[numCards][cash] << endl;
		...
		//finding which cards are in the list
		...
		// while(i > 0 and k > 0){
		// 	cout << i << " " << k << endl;
		// 	if(dynam[i][k] != dynam[i-1][k]){
		// 		cout << "in run" << endl;
		// 		cardsIn.push_back(i);
		// 		i = i-1;
		// 		k = k - her[herNames[i + 1]];
		// 	} else{
		// 		i = i-1;
		// 	}
		// }

		// for(i = 0; i < cardsIn.size(); i++){
		// 	cardsIn[i] = cardsIn[i] -1;
		// }
		// for(i = 0; i < cardsIn.size(); i++){
		// 	cout << cardsIn[i] << endl;
		// }
